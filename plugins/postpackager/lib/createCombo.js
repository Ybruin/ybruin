...
var settings = {
    ...
    mod: projectConf.useModule&&projectConf.useModule.toLowerCase(),
    entry: []
};
...
ycombo.replaceJs = function(ids) {
    ...
    fis.util.map(settings.ret.src, function(subpath, file) {
        ...
        if (file.isViews && file.isHtmlLike && file.filename == settings.file.filename) {
            ...
            switch (settings.mod) {
                case 'amd':
                    replaceContext += '<script>requirejs(["' + settings.entry + '"])</script>';
                    break;
                case 'cmd':
                    replaceContext += '<script>seajs.use("' + settings.entry + '")</script>';
                    break;
                case 'commonjs':
                    replaceContext += '<script>require.async(["' + settings.entry + '"])</script>';
                    break;
            }
